...
function loadRegisteredUsers() {
  ...
  for (const item of list) {
    ...
    registeredUsers.set(String(item.name).toLowerCase(), {
...
function getUserList() {
  return [...registeredUsers.values()]
    .map((user) => {
      const liveEntry = [...users.entries()].find(
...
io.on("connection", (socket) => {
  ...
  socket.on("join", (name) => {
    ...
    const pending = loadJSON(CALLS_FILE);

    const index = pending.findIndex(
      (call) =>
        String(call.targetName).toLowerCase() === String(name).toLowerCase()
    );

    if (index !== -1) {
      const call = pending[index];

      socket.emit("incoming-call-pending", {
        from: call.fromSocketId,
  ...
  socket.on("subscribe-push", (data) => {
    ...
    if (!currentUser || !data || !data.subscription) {
      return;
    }
    ...
    subscriptions = subscriptions.filter(
      (item) => item?.subscription?.endpoint !== data.subscription.endpoint
    );
  ...
  socket.on("chat message", (text) => {
    ...
    saveJSON(MESSAGES_FILE, messages.slice(-1000));

    io.emit("chat message", msg);

    const connected = new Set(
      [...users.values()].map((item) => item.name.toLowerCase())
    );

    for (const registered of registeredUsers.values()) {
      if (!connected.has(registered.name.toLowerCase())) {
        sendPushToUser(registered.name, {
          title: "Mini WhatsApp",
  ...
  socket.on("private message", (data) => {
    ...
    const targetEntry = [...users.entries()].find(
      ([, u]) => String(u.name).toLowerCase() === targetName.toLowerCase()
    );

    if (!targetEntry) {
      socket.emit("private message error", {
        message: "Ese usuario no está conectado.",
      });
      return;
    }
  ...
  socket.on("call-offline", async (data) => {
    ...
    const target = [...registeredUsers.values()].find(
    ...
    const targetSocket = [...users.entries()].find(
      ([, user]) => user.name === target.name
    );

    if (targetSocket) {
      const destino = io.sockets.sockets.get(targetSocket[0]);
    ...
    saveJSON(CALLS_FILE, cleaned.slice(-100));

    await sendPushToUser(target.name, {
      title: call.video ? "Videollamada" : "Llamada",
    ...
    socket.emit("call-pending-saved", {
      targetName: target.name,
  ...
  socket.on("call-pending-accepted", (data) => {
    if (!data?.pendingId) {
      return;
    }
    ...
    calls = calls.filter((call) => String(call.id) !== String(data.pendingId));

    saveJSON(CALLS_FILE, calls);
  });

  socket.on("call-pending-rejected", (data) => {
    if (!data?.pendingId) {
      return;
    }
  ...
  socket.on("new post", (post) => {
    ...
    saveJSON(POSTS_FILE, posts.slice(-200));
